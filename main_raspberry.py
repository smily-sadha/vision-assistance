...
class CompleteVisionAssistant:
    ...
    def __init__(self):
        ...
        # Camera (Raspberry Pi RTSP Stream)
        print("\nStarting camera stream from Raspberry Pi...")

        os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;tcp|max_delay;0|fflags;nobuffer"

        stream_url = "rtsp://10.53.219.99:8554/live"

        self.cap = cv2.VideoCapture(stream_url, cv2.CAP_FFMPEG)
        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        if not self.cap.isOpened():
            print("ERROR: Cannot connect to Raspberry Pi camera stream")
            sys.exit(1)

        print("✓ Connected to Raspberry Pi camera stream")
        ...
        try:
            while True:
                if self.pending_learn_face:
                    ...
                    self.cap = cv2.VideoCapture("rtsp://10.243.224.99:8554/live", cv2.CAP_FFMPEG)
                    self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
